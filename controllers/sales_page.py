...
ResponsiveView:
'''
class PourcentagePV(ScrollView):
    grid_showed = False
    grid = None
    ...
    def show_pourcentage_pv(self):
        if self.grid_showed:
            ...
            self.grid = None
        self.grid = GridLayout(cols=2, spacing=2, size_hint_y=None)
        self.grid.bind(minimum_height=self.grid.setter('height'))
        instance = GestionModel()
        produits = instance.get_pourcentage_produits_vendus
        titles = ('PRODUITS','POURCENTAGE')
...
class SalesPage(MDBoxLayout):
    ...
    def update_produits_en_rupture(self):
        produits_en_rupture = self.gestionmodel.produits_en_rupture
